    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          const double MAX_SPEED_LIMIT=49.5;
          const double MAX_ACC_LIMIT=.224;
          if(too_close_f){
            if (!too_close_l && lane > 0){
              lane --;
            }else if (!too_close_r && lane !=2){
              lane ++;
            }else{
              speed_difference -=MAX_ACC_LIMIT;
            }
          }else{
            if(lane!=1){
              if ( ( lane == 0 && !too_close_r ) || ( lane == 2 && !too_close_l ) ){
                lane = 1;
              }
            }
          ...
          for(int i=1; i < 50 - previous_path_x.size(); i++ ) {
            ref_vel += speed_difference;
              ...
              } else if ( ref_vel < MAX_ACC_LIMIT ) {
                ref_vel = MAX_ACC_LIMIT;
              }
